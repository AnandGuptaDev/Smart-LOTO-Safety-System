...
const char* face_names[FACE_ID_SAVE_NUMBER] = {
    "Unknown",       // ID 0
    "Anand Gupta",   // ID 1
    "Aryan Gahlot",  // ID 2
    "Gaurav Singh",  // ID 3
...
// SD card path for face data log
// With these:
#define FACE_DATA_PATH    "/face_data.bin"
#define EMBEDDING_SIZE    512   // FaceRecognition112V1S8 output size in int8_t
...
// ─────────────────────────────────────────────────────────────
// SD card helpers for face data persistence
// ─────────────────────────────────────────────────────────────
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED

/**
 * Saves the current enrolled face count to the SD card as a
 * simple binary log file.  The actual embedding vectors are
 * stored by the ESP-DL recognizer in the internal flash
 * partition named "fr" (via write_ids_to_flash / set_ids_from_flash).
 * This file acts as a human-readable backup / confirmation that
 * enrollments have been persisted and which slot each name maps to.
 */
void save_face_data_to_sd() {
    if (!SD_MMC.cardType() || SD_MMC.cardType() == CARD_NONE) {
        Serial.println("SD: Card not mounted, skipping save");
        return;
    }

    int count = recognizer.get_enrolled_id_num();
    if (count == 0) {
        Serial.println("SD: No faces enrolled, nothing to save");
        return;
    }

    File f = SD_MMC.open(FACE_DATA_PATH, FILE_WRITE);
    if (!f) {
        Serial.println("SD: Failed to open file for writing");
        return;
    }

    uint32_t magic = 0xFACE0003;
    f.write((uint8_t*)&magic, sizeof(magic));
    f.write((uint8_t*)&count, sizeof(count));

    // Save name table
    for (int i = 0; i < FACE_ID_SAVE_NUMBER; i++) {
        uint8_t len = (uint8_t)strlen(face_names[i]);
        f.write(&len, 1);
        f.write((const uint8_t*)face_names[i], len);
    }

    // Save each embedding as raw floats
    for (int id = 1; id <= count; id++) {
        Tensor<float> &emb = recognizer.get_face_emb(id);
        int emb_size = emb.get_size();  // number of floats
        f.write((uint8_t*)&emb_size, sizeof(emb_size));
        f.write((uint8_t*)emb.get_element_ptr(), emb_size * sizeof(float));
        Serial.printf("SD: Saved ID %d -> %s (%d floats)\n", id,
                      (id < FACE_ID_SAVE_NUMBER) ? face_names[id] : "Unknown",
                      emb_size);
    }

    f.close();
    Serial.printf("SD: Save complete — %d face(s)\n", count);
}

/**
 * Reads the face data log from the SD card on boot.
 * Actual embeddings are already loaded from flash by
 * set_ids_from_flash(); this just prints a confirmation
 * and verifies the log is consistent.
 */
void load_face_data_from_sd() {
    if (!SD_MMC.cardType() || SD_MMC.cardType() == CARD_NONE) {
        Serial.println("SD: Card not mounted");
        return;
    }
    if (!SD_MMC.exists(FACE_DATA_PATH)) {
        Serial.println("SD: No face_data.bin — starting fresh");
        return;
    }

    File f = SD_MMC.open(FACE_DATA_PATH, FILE_READ);
    if (!f) {
    ...
    if (magic != 0xFACE0003) {
        Serial.println("SD: Wrong format — delete face_data.bin and re-enroll");
        f.close();
        return;
    }
    ...
    // Load each embedding and re-enroll into recognizer
    for (int i = 0; i < count; i++) {
        int emb_size = 0;
        f.read((uint8_t*)&emb_size, sizeof(emb_size));

        float *buf = (float*)malloc(emb_size * sizeof(float));
        if (!buf) {
            Serial.printf("SD: malloc failed at ID %d\n", i + 1);
            break;
        }

        f.read((uint8_t*)buf, emb_size * sizeof(float));

        // Build a Tensor<float> from the saved data and enroll it
        Tensor<float> emb;
        emb.set_element(buf).set_shape({emb_size}).set_auto_free(false);

        int assigned_id = recognizer.enroll_id(emb,
                          (i + 1 < FACE_ID_SAVE_NUMBER) ? face_names[i + 1] : "Unknown",
                          false);  // false = don't write to flash

        free(buf);
        Serial.printf("SD: Restored ID %d -> %s\n", assigned_id,
                      (i + 1 < FACE_ID_SAVE_NUMBER) ? face_names[i + 1] : "Unknown");
    }

    f.close();
    Serial.printf("SD: Load complete — %d face(s) restored\n", count);
}
...
static int run_face_recognition(fb_data_t *fb, std::list<dl::detect::result_t> *results)
{
    ...
    if (enrolled_count < FACE_ID_SAVE_NUMBER && is_enrolling) {
        ...
        // ── FIX 1: write to flash ──────────────────────────────
        recognizer.write_ids_to_flash();
        Serial.printf("Enrolled ID: %d -> %s\n", id,
                      (id < FACE_ID_SAVE_NUMBER) ? face_names[id] : "Unknown");

        // ── FIX 2: turn off enrolling after ONE face ───────────
        is_enrolling = 0;

        // ── FIX 3: persist log to SD card ─────────────────────
        save_face_data_to_sd();
    }
   ...
   face_info_t recognize = recognizer.recognize(tensor, landmarks);
    ...
    if (recognize.id >= 0) {
        const char* name = (recognize.id < FACE_ID_SAVE_NUMBER)
                           ? face_names[recognize.id] : "Unknown";
        rgb_printf(fb, FACE_COLOR_GREEN, "%s: %.2f", name, recognize.similarity);
        Serial.printf("Authorized: %s (ID:%d, Score:%.2f)\n",
                      name, recognize.id, recognize.similarity);

       if (millis() - last_oled_update > 2000) {
    last_oled_update = millis();
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_ncenB08_tr);
    u8g2.drawStr(0, 12, "** AUTHORIZED **");
    u8g2.setFont(u8g2_font_ncenB14_tr);
    u8g2.drawStr(0, 36, name);
    u8g2.setFont(u8g2_font_ncenB08_tr);
    char score[20];
    sprintf(score, "Score: %.2f", recognize.similarity);
    u8g2.drawStr(0, 56, score);
    u8g2.sendBuffer();
...
digitalWrite(14, HIGH); // LED OFF
        ...
        Serial.println("UNAUTHORIZED: Face not recognized");

if (millis() - last_oled_update > 2000) {
    last_oled_update = millis();
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_ncenB08_tr);
    u8g2.drawStr(0, 12, "** UNAUTHORIZED **");
    u8g2.setFont(u8g2_font_ncenB14_tr);
    u8g2.drawStr(0, 36, "INTRUDER!");
    u8g2.setFont(u8g2_font_ncenB08_tr);
    u8g2.drawStr(0, 56, "Not recognized");
    u8g2.sendBuffer();
    digitalWrite(14, HIGH); // LED stays OFF
}
    }
    return recognize.id;
}
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    if (fb->format == PIXFORMAT_RGB565
     ...
     && !recognition_enabled
        ...
        if (results.size() > 0) {
            fb_data_t rfb;
            rfb.width = fb->width; rfb.height = fb->height;
            rfb.data = fb->buf; rfb.bytes_per_pixel = 2; rfb.format = FB_RGB565;
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
            detected = true;
#endif
        ...
        out_width = fb->width; out_height = fb->height;
        out_buf = (uint8_t *)malloc(out_len);
        if (!out_buf) { log_e("out_buf malloc failed"); httpd_resp_send_500(req); return ESP_FAIL; }
        s = fmt2rgb888(fb->buf, fb->len, fb->format, out_buf);
        esp_camera_fb_return(fb);
        if (!s) { free(out_buf); log_e("To rgb888 failed"); httpd_resp_send_500(req); return ESP_FAIL; }
        fb_data_t rfb;
        rfb.width = out_width; rfb.height = out_height;
        rfb.data = out_buf; rfb.bytes_per_pixel = 3; rfb.format = FB_BGR888;
#if TWO_STAGE
        HumanFaceDetectMSR01 s1(0.1F, 0.5F, 10, 0.2F);
        HumanFaceDetectMNP01 s2(0.5F, 0.3F, 5);
...
#endif
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
            if (recognition_enabled) face_id = run_face_recognition(&rfb, &results);
#endif
            draw_face_boxes(&rfb, &results, face_id);
        }
    ...
    if (!s) { log_e("JPEG compression failed"); httpd_resp_send_500(req); return ESP_FAIL; }
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
    int64_t fr_end = esp_timer_get_time();
#endif
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    #if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
        bool detected = false;
        int64_t fr_ready = 0, fr_recognize = 0, fr_encode = 0, fr_face = 0, fr_start = 0;
    #endif
    int face_id = 0;
...
#endif
        fb = esp_camera_fb_get();
        if (!fb) { log_e("Camera capture failed"); res = ESP_FAIL; }
        else {
            ...
            fr_ready = fr_start; fr_encode = fr_start;
            fr_recognize = fr_start; fr_face = fr_start;
    #endif
            if (!detection_enabled || fb->width > 400) {
#endif
                if (fb->format != PIXFORMAT_JPEG) {
                ...
                if (fb->format == PIXFORMAT_RGB565
                    ...
                    fr_face = esp_timer_get_time(); fr_recognize = fr_face;
#endif
                    if (results.size() > 0) {
                        fb_data_t rfb;
                        rfb.width = fb->width; rfb.height = fb->height;
                    ...
                    esp_camera_fb_return(fb); fb = NULL;
                    if (!s) { log_e("fmt2jpg failed"); res = ESP_FAIL; }
#if CONFIG_ESP_FACE_DETECT_ENABLED && ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
                    ...
                    out_width = fb->width; out_height = fb->height;
                    out_buf = (uint8_t *)malloc(out_len);
                    if (!out_buf) { log_e("out_buf malloc failed"); res = ESP_FAIL; }
                    else {
                        s = fmt2rgb888(fb->buf, fb->len, fb->format, out_buf);
                        esp_camera_fb_return(fb); fb = NULL;
                        if (!s) { free(out_buf); log_e("To rgb888 failed"); res = ESP_FAIL; }
                        else {
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
                            fr_ready = esp_timer_get_time();
#endif
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    if (!strcmp(variable, "framesize")) { if (s->pixformat == PIXFORMAT_JPEG) res = s->set_framesize(s, (framesize_t)val); }
        ...
        // Toggle: set is_enrolling = 1 to arm ONE enrollment.
        // It is automatically cleared to 0 inside run_face_recognition()
        // after the first face is successfully enrolled.
        is_enrolling = !is_enrolling;
        Serial.printf("Enroll mode: %s\n", is_enrolling ? "ARMED (waiting for face)" : "OFF");
    }
    else if (!strcmp(variable, "face_recognize")) { recognition_enabled = val; if (recognition_enabled) detection_enabled = val; }
#endif
#endif
    else { res = -1; }
...
{
    char *buf = NULL, _reg[32], _mask[32], _val[32];
    if (parse_get(req, &buf) != ESP_OK) return ESP_FAIL;
...
{
    char *buf = NULL, _reg[32], _mask[32];
    if (parse_get(req, &buf) != ESP_OK) return ESP_FAIL;
    if (httpd_query_key_value(buf, "reg", _reg, sizeof(_reg)) != ESP_OK ||
...
static esp_err_t pll_handler(httpd_req_t *req)
{
    ...
    int sys = parse_get_var(buf, "sys", 0), root = parse_get_var(buf, "root", 0);
    int pre = parse_get_var(buf, "pre", 0), seld5 = parse_get_var(buf, "seld5", 0);
...
static esp_err_t win_handler(httpd_req_t *req)
{
    ...
    int ex = parse_get_var(buf, "ex", 0), ey = parse_get_var(buf, "ey", 0);
    int ox = parse_get_var(buf, "offx", 0), oy = parse_get_var(buf, "offy", 0);
    int tx = parse_get_var(buf, "tx", 0), ty = parse_get_var(buf, "ty", 0);
    int ox2 = parse_get_var(buf, "ox", 0), oy2 = parse_get_var(buf, "oy", 0);
    bool scale = parse_get_var(buf, "scale", 0) == 1;
    bool binning = parse_get_var(buf, "binning", 0) == 1;
    free(buf);
    sensor_t *s = esp_camera_sensor_get();
    int res = s->set_res_raw(s, sx, sy, ex, ey, ox, oy, tx, ty, ox2, oy2, scale, binning);
    if (res) return httpd_resp_send_500(req);
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_send(req, NULL, 0);
}
...
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
    // ── Load SD log and verify consistency with flash data ──
    load_face_data_from_sd();
#endif
